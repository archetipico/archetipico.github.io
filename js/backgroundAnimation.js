(() => {
    ...
    function sampleField() {
        const sx = 0.058;
        const sy = 0.052;
        const wt = now * 0.0000055;
        const ot = now * 0.0000009;
        const offX = (noise2(ot + 31.7,  5.1) - 0.5) * 9.0;
        const offY = (noise2(ot - 12.3,  9.4) - 0.5) * 9.0;
        for (let j = 0; j < rows; j++) {
            const yBase = j * sy + offY;
            for (let i = 0; i < cols; i++) {
                const xBase = i * sx + offX;
                const wx = noise2(xBase + 11.3, yBase - 4.7 + wt) - 0.5;
                const wy = noise2(xBase - 7.1,  yBase + 2.9 - wt) - 0.5;
                field[j * cols + i] = fbm(xBase + wx * 1.1 + wt * 0.5, yBase + wy * 1.1 - wt * 0.35);
            }
        }
    }
    ...
    const LEVELS = [0.28, 0.38, 0.46, 0.50, 0.54, 0.62, 0.72];
    ...
    function step(ts) {
        ...
        const phase = (now * 0.0000045) % 1;
        const scanIso = 0.22 + phase * 0.56;
        const scanAlpha = 0.08 + 0.14 * (1 - Math.abs(phase - 0.5) * 2);
        strokeIsoline(
            scanIso,
            `rgba(${tk.mid.r},${tk.mid.g},${tk.mid.b},${scanAlpha})`,
            1.5
        );
